class ReferenceRangesController < ApplicationController
  def index
    @reference_ranges = ReferenceRange.all(:order => "lab_test_id, age_unit, min_age")
  end
  ...
  def create
    ...
    if @reference_range.save
      flash[:notice] = "Successfully created reference_range."
      redirect_to @reference_range
    else
      render :action => 'new'
    end
  end
  ...
  def update
    ...
    if @reference_range.update_attributes(params[:reference_range])
      flash[:notice] = "Successfully updated reference_range."
      redirect_to @reference_range
    else
      render :action => 'edit'
    end
  ...
  def destroy
    ...
    flash[:notice] = "Successfully removed reference_range."
    redirect_to reference_ranges_url
  end
end
