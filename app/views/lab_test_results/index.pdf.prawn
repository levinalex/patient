...
##
# First page header with logo and information
# This part should be in the letterhead of the page
##
...
pdf.bounding_box([125,730], :width => 350, :height => 50) do
  ...
  pdf.text "Villa Lucre, Consultorios San Judas Tadeo, Local 107", :size => 10, :kerning => true
...
pdf.header pdf.margin_box.top_left do
  ...
  pdf.text "Unidades", :size => 8, :style => :bold, :at => [210,610]
...
@results.each do |department, results|
  ...
  pdf.text department, :size => 8, :style => :bold, :align => :left, :at => [0,pdf.cursor-5]
  pdf.text "Run By: #{User.find(@accession.reported_by).initials} on #{@accession.reported_at.strftime('%d/%m/%Y %I:%M %p')}", :size => 7.5, :style => :italic, :align => :right
  pdf.table results_table,
    :headers => [Prawn::Table::Cell.new( :text => department, :borders => [:none], :align => :left, :at => [0,pdf.cursor-5] )],
    :font_size => 8,
    :vertical_padding => 2,
...
pdf.move_down(15)
pdf.text "Revisado por:", :size => 8, :at => [70,pdf.cursor]
...
pdf.footer [0,40] do
  pdf.font "Helvetica" do
    ...
    pdf.stroke_horizontal_rule
    pdf.move_down 5
    pdf.text "Fecha original de impresión: #{@accession.reported_at.strftime('%d/%m/%Y %I:%M %p')}", :size => 8
    pdf.text "Fecha de impresión: #{Time.now.strftime('%d/%m/%Y %I:%M %p')}", :size => 8
    pdf.move_up 18
    ...
    # #{pdf.page_count}
    ##
    pdf.text "Página "+num_pag.to_s+" de "+pdf.page_count.to_s, :size => 8, :align => :left
  end
end
